...
shinyUI(fluidPage(
  ...
  fluidRow( 
    column(4, fileInput("upload", "Interactome Data", accept = c(".csv", "xls","xlsx", ".Rds"))),
    column(4, fileInput("expmtr", "Expression Matrix (For Violin Plots only)", accept = c(".Rds"))),
    column(4, fileInput("meta", "Meta Data (For Violin Plots only)", accept = c(".csv", "xls", "xlsx", ".Rds")))),
                             ...
                             sidebarPanel(
                               ...
                               tabsetPanel(id="tabs",
                                           tabPanel('Interactions',
                                                    br(),
                                                    selectInput('interaction', label = 'Select interaction', choices = sort(unique(paste0(data()$ligand, '|', data()$receptor)))),
                                                    hr(),
                                           ...
                                           tabPanel('Cell types',
                                                    br(),
                                                    selectInput('celltype', label = 'Select cell type', choices = sort(unique(c(data()$source, data()$target)))),
                                                    hr(),
                                           ...
                                           tabPanel('Genes',
                                                    br(),
                                                    selectInput('gene', label = 'Select gene', choices = sort(unique(c(data()$ligand, data()$receptor))), multiple = T, selected = 'ADAM10'),
                                                    hr(),
                           ...
                           sidebarLayout(
                             sidebarPanel(
                               ...
                               tabsetPanel(id="tabs2",
                                           tabPanel('Cell Abundance',
                                                    hr(),
                                                    radioButtons('cell_abundance', 'Select plot type', choices = c('Healthy', 'Diagnosis', 'Relapse', 'Post-Treatment'), selected = 'Healthy')
                                                    
                                           ),
                                           tabPanel('Top Interactions',
                                                    hr(),
                                                    radioButtons('top_interactions', 'Select plot type', choices = c('Healty', 'Diagnosis', 'Relapse', 'Post-Treatment'), selected = 'Healty'),
                                           )
                                           
                                  )
                             ),
                             mainPanel(
                               conditionalPanel(condition="input.tabs2 == 'Cell Abundance'",
                                                withSpinner(plotlyOutput('cell_abun', width = '100%', height = "600px"), color="#9FDC93", size = 0.5)
                               ),
                               conditionalPanel(condition="input.tabs2 == 'Top Interactions'",
                                                withSpinner(plotOutput('top_int', width = '100%', height = "600px"), color="#9FDC93", size = 0.5)
                               ),
                             )))
              ),
